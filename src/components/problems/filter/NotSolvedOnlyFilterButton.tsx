...
interface NotSolvedOnlyFilterButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement>{
    onClick: () => void;
    notSolvedOnly: boolean;
}

const NotSolvedOnlyFilterButton = ({notSolvedOnly, onClick}: NotSolvedOnlyFilterButtonProps) => {
    return (
        <NotSolvedOnlyFilterButtonStyle $notSolvedOnly={notSolvedOnly}>
            <button onClick={onClick}>
                {
                    notSolvedOnly ? <MdOutlineCheckBox /> : <MdOutlineCheckBoxOutlineBlank />
                }
                안 푼 문제만
            </button>
...
export const NotSolvedOnlyFilterButtonStyle = styled.div<INotSolvedOnlyFilterButtonStyleProps>`
    button {
        ...
        color: ${({ theme, $notSolvedOnly }) => $notSolvedOnly ? theme.color.text2 : theme.color.text1};
